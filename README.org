...
3. Create a new tar archive of the temporary GnuPG home directory with an incremented number
   #+begin_src shell
     tar cf gnupg.$(find -maxdepth 1 -name '*.tar' | wc -l).tar gnupg-wd
   #+end_src
...
2. Determine the fingerprints of the primary private keys. Extract all files from the current archive, without the primary private keys:
   #+begin_src shell :session manage-pgphome
     gnupg_kg=$(gpg -K --with-keygrip \
     	       | grep -A2 -e 'sec' \
     	       | grep -e 'Keygrip = ' \
     	       | cut -d '=' -f2 \
     	       | tr -d ' ')
     tar -C $PWD/gnupg-tmp --exclude=private-keys-v1.d/$gnupg_kg.key \
         -xf gnupg.$(($(find -maxdepth 1 -name '*.tar' | wc -l) - 1)).tar
     unset gnupg_kg
   #+end_src
