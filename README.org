...
The file =README.org= is this file. The files =gnupg.<x>.tar= are backups, where =gnupg.0.tar= is the first backup and =gnupg.N.tar= the most current one.
...
#+begin_example shell
  sudo mount -t drvfs -o uid=$(id -u $USER),gid=$(id -g $USER) d: /media/$USER
#+end_example
...
To manage the keys, they usually have to be restored from a backup first.:
#+begin_src shell :session manage-pgphome
  tar -xf bak/gnupg.$(($(find bak -maxdepth 1 -name '*.tar' | wc -l) - 1)).tar
#+end_src

* Create backup
After managing the keys, a new backup of the keys must be created. The backups are stored in tar archives starting with =gnupg.0.tar=. The next backup has then :
...
1. Extract all files from backup into the temporary directory:
   #+begin_src shell :session manage-pgphome
     tar -xf bak/gnupg.$(($(find bak -maxdepth 1 -name '*.tar' | wc -l) - 1)).tar
   #+end_src

2. Create a temporary directory to extract files to:
   #+begin_src shell :session manage-pgphome
     mkdir -p ~/gnupg-tmp
   #+end_src

3. Determine the fingerprints of the primary private keys. Extract all files from the current archive, without the primary private keys:
   #+begin_src shell :session manage-pgphome
     gnupg_exclusions=$(gpg -K --with-keygrip \
     	| grep -e '^sec ' -A 2 \
     	| sed -nE 's/ *Keygrip = ([0-9A-F]+)/--exclude=private-keys-v1.d\/\1.key/p')
     tar -C ~/gnupg-tmp $gnupg_exclusions \
         -xf bak/gnupg.$(($(find bak -maxdepth 1 -name '*.tar' | wc -l) - 1)).tar
     unset gnupg_exclusions
   #+end_src

4. Make the extracted files the new target GnuPG homedir =$HOME/.gnupg=:
   #+begin_src shell :session manage-pgphome
     mv ~/gnupg-tmp/gnupg-wd ~/.gnupg
   #+end_src
...
#+begin_example shell :results silent
